...
export default function Blog() {
  ...
  // Categories extraction based on data + defaults
  const categories = ['All', 'Business Systems', 'HR & Team', 'AI Consulting', 'Leadership', 'Case Studies']

  // Featured Blog 
  const featuredBlog = blogs.find(b => b.category === 'Case Studies') || blogs[0]
    ...
    <>
      ...
      <section className="py-20 px-[4%] bg-[#FAF8F5] relative z-20">
        <div className="max-w-7xl mx-auto">
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={viewportOptions} className="mb-10">
            <SectionLabel>Featured Story</SectionLabel>
            <h2 className="section-heading text-navy">Operational Case Study</h2>
          </motion.div>

          {loading ? (
            <div className="w-full h-96 bg-navy/5 rounded-3xl animate-pulse" />
          ) : featuredBlog ? (
            <motion.div 
            ...
            >
              <div className="lg:col-span-6 relative overflow-hidden group min-h-[300px] lg:min-h-full bg-navy/5">
                <img 
              ...
              <div className="lg:col-span-6 p-8 lg:p-12 flex flex-col justify-between">
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-8">
                    {featuredBlog.tags && featuredBlog.tags.map(t => (
            ...
            <div className="p-12 text-center text-navy/50 bg-white rounded-3xl border border-gold/15">No featured articles available.</div>
          )}
        </div>
        ...
        <div className="max-w-7xl mx-auto relative z-20">
          ...
          {loading ? (
            ...
            <AnimatePresence mode="wait">
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 15 }} className="text-center py-20 bg-white/[0.01] border border-white/5 rounded-2xl p-8 relative z-10">
                <p className="text-white/40 text-base">No articles found matching your criteria.</p>
                <button onClick={() => { setSearchQuery(''); setSelectedCategory('All'); }} className="text-gold text-sm underline mt-3 hover:text-white transition-colors">
      ...
      <AnimatePresence>
        {selectedBlog && (
          <div className="fixed inset-0 z-50 overflow-hidden flex justify-end">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={handleCloseReader} className="absolute inset-0 bg-black/85 backdrop-blur-md cursor-pointer" />

            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 220 }} className="relative w-full max-w-3xl h-full bg-[#0B1120] border-l border-gold/20 shadow-2xl flex flex-col z-10">
              ...
              <div className="sticky top-0 bg-[#0B1120]/90 backdrop-blur-md border-b border-white/10 px-6 py-4 flex items-center justify-between z-30">
                ...
                <button onClick={handleCloseReader} className="w-10 h-10 rounded-full border border-white/10 flex items-center justify-center text-white/50 hover:text-white hover:border-white/20 transition-all duration-300">
                  <X size={18} />
                </button>
              </div>

              <div className="flex-1 overflow-y-auto px-6 py-8 md:p-12 space-y-8 select-text">
                <div className="w-full aspect-[2/1] rounded-2xl overflow-hidden border border-white/5 relative bg-white/5">
                  <img src={selectedBlog.image} alt={selectedBlog.title} className="w-full h-full object-cover" onError={(e) => {e.target.onerror = null; e.target.src = '/images/placeholder.jpg'}} />
                ...
                <div className="space-y-6 text-white/70 text-sm md:text-base leading-relaxed font-light">
                  ...
                  {selectedBlog.content?.sections && selectedBlog.content.sections.map((sec, i) => (
                    <div key={i} className="space-y-2 pt-4">
                      <h3 className="font-serif text-xl font-semibold text-white tracking-wide">{sec.heading}</h3>
                  ...
                  {selectedBlog.content?.metrics && selectedBlog.content.metrics.length > 0 && (
                ...
                <div className="pt-10 border-t border-white/10 flex flex-col items-center text-center space-y-4">
                  ...
                  <button onClick={() => { handleCloseReader(); openModal('', `I am interested in setting up frameworks similar to the ones discussed in: ${selectedBlog.title}`) }} className="btn-gold py-3 px-8 text-xs font-semibold tracking-wider">
                    Schedule Free Audit
                  </button>
                </div>
